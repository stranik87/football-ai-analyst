...
from app.analytics.team_scoring_pattern_analyzer import (
    TeamScoringPatternAnalyzer,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Анализ распределения голов команды по таймам"
    )
...
def main() -> int:
    ...
    try:
        ...
        analyzer = TeamScoringPatternAnalyzer(
            session=session
        )
        ...
        logger.exception(
            "Ошибка анализа распределения голов"
        )
        return 1
