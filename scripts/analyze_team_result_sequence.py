...
from app.analytics.team_result_sequence_analyzer import (
    TeamResultSequenceAnalyzer,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Анализ последовательности результатов команды"
    )
...
def main() -> int:
    ...
    try:
        ...
        analyzer = TeamResultSequenceAnalyzer(
            session=session
        )
        ...
        logger.exception(
            "Ошибка анализа последовательности результатов"
        )
        return 1
